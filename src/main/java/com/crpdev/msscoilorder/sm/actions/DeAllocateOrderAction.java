...
@Component
public class DeAllocateOrderAction implements Action<OilOrderStatusEnum, OilOrderEventEnum> {
    ...
    public void execute(StateContext<OilOrderStatusEnum, OilOrderEventEnum> stateContext) {
        log.debug("<<< Received De-Allocation Request");
        ...
        log.debug("<<< Processing De-Allocation Request For Order Id: " + oilOrder.getId() + " with Status: "  + oilOrder.getOrderStatus());
        jmsTemplate.convertAndSend(JmsConfig.DEALLOCATE_ORDER_QUEUE, AllocateOrderRequest.builder()
                    .oilOrderDto(oilOrderMapper.toOilOrderDto(oilOrder)).build());

        log.debug("De-Allocation request sent for order id: " + oilOrderId);
    }
}
